...
void App::SetWindow(CoreWindow^ window)
{
    ...
    m_gestureRecognizer->ManipulationUpdated +=
        ref new TypedEventHandler<GestureRecognizer^, ManipulationUpdatedEventArgs^>(this, &App::OnManipulationUpdated);
...
void App::Load(Platform::String^ entryPoint)
{
    if (m_main == nullptr)
    {
        m_main = std::unique_ptr<CustomVertexShaderMain>(new CustomVertexShaderMain(m_deviceResources));
    }
}
...
{
    m_main->UpdateManipulationState(args);
}
